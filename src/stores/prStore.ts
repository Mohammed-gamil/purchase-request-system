...
interface PRActions {
  ...
  createRequest: (requestData: CreateRequestForm) => Promise<Request>;
      ...
      getPRs: async () => {
        try {
          ...
          if (user?.role === 'ADMIN') {
            ...
            // All non-admin roles see only their own requests in /prs
            requests = await requestService.getUserRequests(String(user.id));
          }
