...
export default function PRList() {
  ...
  const filteredPRs = prs.filter(pr => {
    const titleText = (pr as any)?.title ? String((pr as any).title).toLowerCase() : '';
    const idText = String((pr as any)?.request_id ?? (pr as any)?.id ?? '').toLowerCase();
    const q = searchQuery.toLowerCase();

    const matchesSearch = q === '' || titleText.includes(q) || idText.includes(q);
    const matchesState = stateFilter === 'all' || (pr as any)?.state === stateFilter;
        ...
        <CardContent>
          {filteredPRs.length > 0 ? (
            <div className="pr-card-grid">
              {filteredPRs.map((pr) => {
                const neededDate = (pr as any).needed_by_date
                  ? new Date((pr as any).needed_by_date)
                  : (pr as any).neededByDate
                  ? new Date((pr as any).neededByDate)
                  : null;
                const desiredCost = (pr as any).desired_cost ?? (pr as any).desiredCost ?? 0;
                const currency = (pr as any).currency ?? 'USD';
                const idStr = String((pr as any).id);
                const initials = (pr as any)?.requester?.name
                  ? String((pr as any).requester.name)
                      .split(/\s+/)
                      .map((n: string) => n[0])
                      .join('')
                      .toUpperCase()
                ...
                return (
                <div key={idStr} className="pr-card">
                  {/* Card Header */}
                  ...
                  <div className="pr-card-footer">
                    <div className="pr-card-metadata">
                      <div className="metadata-item">
                        ...
                        <span>{t('prDetails.neededBy')} {neededDate ? format(neededDate, 'MMM d') : '-'}</span>
                      </div>
                      <div className="metadata-item">
                        <DollarSign className="metadata-icon" />
                        <span>{formatCurrency(desiredCost as number, currency as string)}</span>
                      </div>
                      <div className="metadata-item">
