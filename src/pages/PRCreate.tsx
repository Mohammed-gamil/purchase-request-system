...
const baseFormSchema = z.object({
  ...
  description: z.string().min(1, "prCreate.errorDescriptionRequired").max(5000, "prCreate.errorDescriptionMax"),
  ...
  neededByDate: z.date().min(new Date(Date.now() + 24 * 60 * 60 * 1000), "prCreate.errorNeededByDate"),
});
...
export default function PRCreate() {
  ...
  const onSubmit = async (data: PRFormData) => {
    try {
      ...
      // Navigate to the created PR details for clarity
      if (created?.id) {
        navigate(`/prs/${created.id}`);
      } else {
        navigate('/prs');
      }
