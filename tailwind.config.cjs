/* CommonJS Tailwind configuration generated to ensure PostCSS/Tailwind can load a JS config at runtime.
   Mirrors the project's existing `tailwind.config.ts` so utilities like `bg-background` and
   `border-border` are generated during Vite dev builds.
*/
...
module.exports = {
  ...
  theme: {
    ...
    extend: {
      colors: {
        // Use CSS variables so edits in src/index.css are reflected without rebuilding Tailwind
        border: 'var(--border)',
        input: 'var(--input)',
        ring: 'var(--ring)',
      ...
      keyframes: {
        ...
        'bounce-subtle': { '0%, 100%': { transform: 'translateY(-2px)' }, '50%': { transform: 'translateY(0)' } },
        'pulse-soft': { '0%, 100%': { opacity: '1' }, '50%': { opacity: '0.8' } },
        'shimmer': { '0%': { backgroundPosition: '-200% 0' }, '100%': { backgroundPosition: '200% 0' } },
        'glow': { '0%, 100%': { boxShadow: '0 0 20px rgba(37, 99, 235, 0.3)' }, '50%': { boxShadow: '0 0 30px rgba(37, 99, 235, 0.5)' } }
      },
      animation: {
